  ...
  if (!perfil) return (
    <Layout>
      <div className="text-center py-16">
        ...
        <p className="text-slate-600">{error || "Consumidor no encontrado."}</p>
      </div>
    </Layout>
  );

  const textareaCls = "w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none resize-none";
    ...
    <Layout>
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/anh/consumidores")} className="p-2 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-500 text-sm">{perfil.user.email}</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-slate-700">Verificación de identidad</h2>
            </div>
            <EstadoSolicitudBadge estado={perfil.estado_identidad} />
          </div>
          <div className="px-6 py-4">
            {!accionIdentidad ? (
              ...
              <div className="space-y-3">
                <p className="text-sm font-medium text-slate-700">
                  Cambiar a: <span className="text-blue-600">{accionIdentidad}</span>
                </p>
                <textarea
                  value={motivo}
                ...
                />
                <div className="flex gap-2">
                  <button onClick={() => setAccionIdentidad(null)} className="px-3 py-2 border border-slate-200 text-slate-600 rounded-xl text-xs hover:bg-slate-50 transition-colors">
                    Cancelar
                  </button>
                  <button onClick={() => cambiarIdentidad(accionIdentidad)} disabled={procesando}
                    className="flex items-center gap-2 px-4 py-2 bg-[#1a3a5c] text-white rounded-xl text-xs font-medium hover:bg-[#152e4d] disabled:bg-slate-300 transition-colors">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-slate-700">Alerta de repetitividad</h2>
            </div>
            <AlertaBadge alerta={perfil.alerta_repetitividad} />
          </div>
          <div className="px-6 py-4">
            {perfil.motivo_bloqueo && (
              <p className="text-sm text-slate-600 bg-amber-50 rounded-xl px-4 py-2 mb-3">
                {perfil.motivo_bloqueo}
              </p>
            )}
            {perfil.fecha_alerta && (
            ...
            {!accionAlerta ? (
              ...
              <div className="space-y-3">
                <p className="text-sm font-medium text-slate-700">
                  Cambiar alerta a: <span className="text-blue-600">{accionAlerta}</span>
                </p>
                <textarea
                  value={motivo}
                  onChange={e => setMotivo(e.target.value)}
                ...
                />
                <div className="flex gap-2">
                  <button onClick={() => setAccionAlerta(null)} className="px-3 py-2 border border-slate-200 text-slate-600 rounded-xl text-xs hover:bg-slate-50 transition-colors">
                    Cancelar
                  </button>
        ...
        {perfil.documentos.length > 0 && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-slate-700">Documentos de identidad</h2>
            </div>
            {perfil.documentos.map(doc => (
              <div key={doc.id} className="px-6 py-4 border-b border-slate-100 last:border-0">
                <p className="text-sm font-semibold text-slate-700 mb-1">{doc.tipo_documento_display}</p>
